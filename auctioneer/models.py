...
class AuctionFranchise(models.Model):
    ...
    def __str__(self):
        return f'{self.original_franchise.user.username} in {self.auction.name}'
...
class AuctionPlayer(models.Model):
    ...
    interested_team = models.ManyToManyField(AuctionFranchise,null=True,blank=True, related_name="interested_in")
